...
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Champion;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Champion.Cache;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Item;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Item.Cache;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.LanguageStrings;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.LanguageStrings.Cache;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Map;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Map.Cache;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Mastery;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Mastery.Cache;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Realm;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Realm.Cache;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Rune;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.Rune.Cache;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.SummonerSpell;
using Sihirdar.WebService.Provider.RiotApi.Model.StaticDataEndpoint.SummonerSpell.Cache;
...
namespace Sihirdar.WebService.Provider.RiotApi
{
    public class StaticRiotApi : IStaticRiotApi
    {
        ...
        private readonly TimeSpan _defaultSlidingExpiry = new TimeSpan(0, 30, 0);
        #endregion

        public StaticRiotApi(RiotApiConfig config, ICache cache)
        {
            Requesters.StaticApiRequester = new Requester(config.ApiKey);
            _requester = Requesters.StaticApiRequester;
            _cache = cache;
        }
        ...
        {
            var wrapper = _cache.Get<string, ChampionListStaticWrapper>(ChampionsCacheKey);
            if (wrapper == null || language != wrapper.Language || championData != wrapper.ChampionData)
            {
                ...
                _cache.Add(ChampionsCacheKey, wrapper, _defaultSlidingExpiry);
            }
        ...
        public ChampionStatic GetChampion(Region region, int championId,
            ChampionData championData = ChampionData.basic, Language language = Language.en_US)
        {
            var wrapper = _cache.Get<string, ChampionStaticWrapper>(ChampionCacheKey + championId);
            ...
            {
                var listWrapper = _cache.Get<string, ChampionListStaticWrapper>(ChampionsCacheKey);
                ...
                else
                {
                    var json = _requester.CreateGetRequest(
                        ...
                        new List<string>
                        {
                            $"locale={language}",
                            championData == ChampionData.basic ?
                            string.Empty : $"champData={championData}"
                        });
                    var champ = JsonConvert.DeserializeObject<ChampionStatic>(json);
                    _cache.Add(ChampionCacheKey + championId, new ChampionStaticWrapper(champ, language, championData),
                        _defaultSlidingExpiry);
                    return champ;
                }
            }
        }
        ...
        {
            var wrapper = _cache.Get<string, ItemListStaticWrapper>(ItemsCacheKey);
            if (wrapper == null || language != wrapper.Language || itemData != wrapper.ItemData)
            {
                ...
                _cache.Add(ItemsCacheKey, wrapper, _defaultSlidingExpiry);
            }
        ...
        public ItemStatic GetItem(Region region, int itemId, ItemData itemData = ItemData.basic,
            Language language = Language.en_US)
        {
            var wrapper = _cache.Get<string, ItemStaticWrapper>(ItemCacheKey + itemId);
            ...
            {
                var listWrapper = _cache.Get<string, ItemListStaticWrapper>(ItemsCacheKey);
                ...
                else
                {
                    ...
                    _cache.Add(ItemCacheKey + itemId, new ItemStaticWrapper(item, language, itemData),
                        _defaultSlidingExpiry);
                    return item;
                }
            }
            ...
            Language language = Language.en_US)
        {
            ...
            _cache.Add(ItemCacheKey + itemId, new ItemStaticWrapper(item, language, itemData), _defaultSlidingExpiry);
            return item;
        }
        ...
        public LanguageStringsStatic GetLanguageStrings(Region region, Language language = Language.en_US,
            string version = "")
        {
            var wrapper = _cache.Get<string, LanguageStringsStaticWrapper>(LanguageStringsCacheKey);
            if (wrapper != null && wrapper.Language == language && wrapper.Version == version)
            {
                return wrapper.LanguageStringsStatic;
            }

            var json = _requester.CreateGetRequest(string.Format(LanguageStringsRootUrl, region.ToString()), RootDomain,
                new List<string> {
                    string.Format("locale={0}", language.ToString()),
            ...
            _cache.Add(LanguageStringsCacheKey, new LanguageStringsStaticWrapper(languageStrings,
                language, version), _defaultSlidingExpiry);
            ...
            Language language = Language.en_US, string version = "")
        {
            ...
            var json = await _requester.CreateGetRequestAsync(string.Format(LanguageStringsRootUrl, region.ToString()),
        ...
        {
            var wrapper = _cache.Get<string, List<Language>>(LanguagesCacheKey);
            if (wrapper != null)
            {
                return wrapper;
            }

            var json = _requester.CreateGetRequest(string.Format(LanguagesRootUrl, region.ToString()), RootDomain);
            var languages = JsonConvert.DeserializeObject<List<Language>>(json);

            _cache.Add(LanguagesCacheKey, languages, _defaultSlidingExpiry);
        ...
        public async Task<List<Language>> GetLanguagesAsync(Region region)
        {
            ...
            var json = await _requester.CreateGetRequestAsync(string.Format(LanguagesRootUrl, region.ToString()),
                RootDomain);
        ...
        {
            var wrapper = _cache.Get<string, MapsStaticWrapper>(MapCacheKey);
            if (wrapper != null && wrapper.Language == language && wrapper.Version == version)
            {
                return wrapper.MapsStatic.Data.Values.ToList();
            }

            var json = _requester.CreateGetRequest(string.Format(MapRootUrl, region.ToString()), RootDomain,
                new List<string> {
                    string.Format("locale={0}", language.ToString()),
                    string.Format("version={0}", version)
                });
            ...
            _cache.Add(MapCacheKey, new MapsStaticWrapper(maps, language, version), _defaultSlidingExpiry);
            ...
            string version = "")
        {
            ...
            var json = await _requester.CreateGetRequestAsync(string.Format(MapRootUrl, region.ToString()), RootDomain,
                new List<string> {
                    string.Format("locale={0}", language.ToString()),
                    string.Format("version={0}", version)
                });
        ...
        public MasteryListStatic GetMasteries(Region region, MasteryData masteryData = MasteryData.basic,
            Language language = Language.en_US)
        {
            var wrapper = _cache.Get<string, MasteryListStaticWrapper>(MasteriesCacheKey);
            if (wrapper == null || language != wrapper.Language || masteryData != wrapper.MasteryData)
            {
                ...
                _cache.Add(MasteriesCacheKey, wrapper, _defaultSlidingExpiry);
            }
        ...
        public MasteryStatic GetMastery(Region region, int masteryId, MasteryData masteryData = MasteryData.basic,
            Language language = Language.en_US)
        {
            var wrapper = _cache.Get<string, MasteryStaticWrapper>(MasteryCacheKey + masteryId);
            ...
            {
                var listWrapper = _cache.Get<string, MasteryListStaticWrapper>(MasteriesCacheKey);
                ...
                else
                {
                    ...
                    _cache.Add(MasteryCacheKey + masteryId, new MasteryStaticWrapper(mastery, language, masteryData),
                        _defaultSlidingExpiry);
                    return mastery;
                }
            }
        ...
        {
            var wrapper = _cache.Get<string, RealmStaticWrapper>(RealmCacheKey);
            if (wrapper != null)
            {
                return wrapper.RealmStatic;
            }

            var json = _requester.CreateGetRequest(string.Format(RealmRootUrl, region.ToString()), RootDomain);
            var realm = JsonConvert.DeserializeObject<RealmStatic>(json);

            _cache.Add(RealmCacheKey, new RealmStaticWrapper(realm), _defaultSlidingExpiry);
        ...
        public async Task<RealmStatic> GetRealmAsync(Region region)
        {
            ...
            var json = await _requester.CreateGetRequestAsync(string.Format(RealmRootUrl, region.ToString()), RootDomain);
        ...
        public RuneListStatic GetRunes(Region region, RuneData runeData = RuneData.basic
            , Language language = Language.en_US)
        {
            var wrapper = _cache.Get<string, RuneListStaticWrapper>(RunesCacheKey);
            if (wrapper == null || language != wrapper.Language || runeData != wrapper.RuneData)
            {
                ...
                _cache.Add(RunesCacheKey, wrapper, _defaultSlidingExpiry);
            }
        ...
        public RuneStatic GetRune(Region region, int runeId, RuneData runeData = RuneData.basic,
            Language language = Language.en_US)
        {
            var wrapper = _cache.Get<string, RuneStaticWrapper>(RuneCacheKey + runeId);
            ...
            {
                var listWrapper = _cache.Get<string, RuneListStaticWrapper>(RunesCacheKey);
                ...
                else
                {
                    ...
                    _cache.Add(RuneCacheKey + runeId, new RuneStaticWrapper(rune, language, runeData),
                        _defaultSlidingExpiry);
                    return rune;
                }
            }
            ...
            Language language = Language.en_US)
        {
            ...
            _cache.Add(RuneCacheKey + runeId, new RuneStaticWrapper(rune, language, runeData), _defaultSlidingExpiry);
            return rune;
        }
        ...
        {
            var wrapper = _cache.Get<string, SummonerSpellListStaticWrapper>(SummonerSpellsCacheKey);
            if (wrapper == null || wrapper.Language != language || wrapper.SummonerSpellData != summonerSpellData)
            {
                ...
                _cache.Add(SummonerSpellsCacheKey, wrapper, _defaultSlidingExpiry);
            }
        ...
        public SummonerSpellStatic GetSummonerSpell(Region region, SummonerSpell summonerSpell,
            SummonerSpellData summonerSpellData = SummonerSpellData.basic, Language language = Language.en_US)
        {
            var wrapper = _cache.Get<string, SummonerSpellStaticWrapper>(
            ...
            {
                var listWrapper = _cache.Get<string, SummonerSpellListStaticWrapper>(SummonerSpellsCacheKey);
                ...
                else
                {
                    ...
                    _cache.Add(SummonerSpellCacheKey + summonerSpell.ToCustomString(),
                        new SummonerSpellStaticWrapper(spell, language, summonerSpellData), _defaultSlidingExpiry);
                    return spell;
                }
            }
        }
        ...
        {
            var wrapper = _cache.Get<string, List<string>>(VersionCacheKey);
            if (wrapper != null)
            {
                return wrapper;
            }

            var json = _requester.CreateGetRequest(string.Format(VersionRootUrl, region.ToString()), RootDomain);
            var version = JsonConvert.DeserializeObject<List<string>>(json);

            _cache.Add(VersionCacheKey, version, _defaultSlidingExpiry);
        ...
        public async Task<List<string>> GetVersionsAsync(Region region)
        {
            ...
            var json =
                await _requester.CreateGetRequestAsync(string.Format(VersionRootUrl, region.ToString()), RootDomain);
